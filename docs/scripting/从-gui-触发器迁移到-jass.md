...
sources:
  ...
  - https://world-editor-tutorials.thehelper.net/cat_usersubmit.php?view=25715
...
本节将帮助你理解**GUI触发器**（图形界面触发器）和**JASS脚本**之间的区别，以及为什么随着地图复杂度提升，你需要考虑学习JASS编程。学完本节后，你将能够判断自己的地图是否需要使用JASS，并了解迁移后将获得哪些能力提升。
...
**功能更强大**：JASS可以实现内存泄漏优化、自定义数据结构、代码模块化复用等高级特性[^2]。这也是制作高质量RPG地图的必备技能。

**可维护性更强**：代码可以复制、分享和版本管理，方便学习和改进他人的优秀实现[^5]。
...
- **大量自定义单位/物品**：需要精确控制每个单位的属性和行为
- **性能要求高**：地图规模大、玩家数量多，需要优化运行效率
...
JASS是Warcraft III的原生脚本语言，全称是 "Journal of Assumed Scripting Syntax"（或被玩家戏称为"Just Another Scripting Syntax"）。[^1] 你可以把JASS理解为世界编辑器的"内部语言"——当你用GUI触发器（拖拽式的可视化编程界面）创建功能时，编辑器实际上在后台生成了对应的JASS代码。[^6]
...
3. **查看生成的代码** — 开启后，每个触发器右侧会显示对应的JASS脚本代码区域
4. **打开编辑界面** — 双击触发器右侧的代码区域，即可进入JASS代码编辑界面

> **⚠️ 常见错误**：很多新手忘记开启"显示无头JASS"选项，导致看不到任何JASS代码。如果你打开触发器编辑器后发现右侧没有代码区域显示，第一时间检查这个选项是否已勾选。
...
2. **认识变量赋值** — 给变量设置值使用`set`关键字，例如`set udg_MyVariable = 100`，注意变量名前通常带有`udg_`前缀。[^4]
...
在World Editor中，GUI触发器分为三个部分：**事件（Event）**告诉电脑"什么时候"执行代码，**条件（Condition）**告诉电脑"要不要"执行，**动作（Action）**则是具体要做的事情。简单来说，事件是"开关"，条件是"检查站"，动作是"执行任务"。
...
function InitTrig_地图初始化 takes nothing returns nothing
    set gg_trg_地图初始化 = CreateTrigger()
    call TriggerAddAction(gg_trg_地图初始化, function Trig_地图初始化_Actions)
endfunction
```
...
| GUI动作 | JASS写法 |
|---------|----------|
| 显示文本给玩家 | `DisplayTextToForce(玩家组, "文字内容")` |
| 设置变量值 | `set 变量名 = 数值` |
| 创建单位 | `CreateUnitAtLoc(玩家, 单位类型, 位置, 面向角度)` |
| 添加效果 | `AddSpecialEffectLoc(效果路径, 位置)` |
| 伤害单位 | `UnitDamageTarget(攻击者, 目标, 伤害值, 攻击类型, 伤害类型, 是否近战)` |

> **⚠️ 常见错误**：很多新手把JASS函数名记错，比如把 `CreateUnit` 记成 `MakeUnit` 或 `SpawnUnit`。建议打开JASS手册[^5]或World Editor Tutorials的基础教程[^4]查阅正确写法，养成先查文档再写代码的习惯。
...
```jass
function MyFunction takes nothing returns nothing
    local unit u  // 声明一个名为u的单位变量
    local integer counter  // 声明一个名为counter的整数变量
    
    set u = GetTriggerUnit()  // 给u赋值
    set counter = 0  // 给counter赋初始值
endfunction
```

这里 `local` 表示"局部变量"，只在当前函数内部有效。函数是一段可以重复使用的代码块，用 `function 函数名 takes 参数 returns 返回类型` 开始，用 `endfunction` 结束。

**JASS的基本结构模板：**
```jass
function 函数名 takes 参数类型 参数名 returns 返回类型
    // 你的代码写在这里
    return 返回值  // 如果返回类型是nothing，这行省略
endfunction
```

> **💡 新手提示**：初学阶段，建议先在GUI中做好触发器，然后用编辑器自带的"转换为JASS"功能（右键触发器选择"转换为文本"）查看生成的代码，对照理解每个部分是什么意思。这是学习JASS最快的方法。

### 小结

通过本节学习，你应该掌握了GUI三大结构（事件-条件-动作）对应的JASS写法，了解了常用GUI动作的JASS替代方案，并认识了变量声明和函数的基本语法。现在你可以尝试打开World Editor，选一个简单的GUI触发器，用编辑器自带的转换功能看看它对应的JASS代码长什么样，培养对JASS代码的直观感觉。
...
1. **打开JASS脚本编辑器** — 在World Editor菜单栏中点击「触发器编辑器」（或按F4），然后在左侧找到你的触发器并右键选择「转换为自定义脚本」[^1]
2. **定义一个新函数** — 输入以下代码框架：
...
> **💡 新手提示**：函数名后面的 `takes` 后面跟的是参数类型，`returns` 后面跟的是返回值类型。如果你不需要任何参数和返回值，就用 `nothing`（相当于"空"）。[^*]

> **⚠️ 常见错误**：很多新手会忘记写 `endfunction`，或者把函数名写成 `My Function`（中间有空格）。JASS中函数名不能有空格，必须是一个连续的单词。

### 原生函数(Native Functions)的使用

**原生函数（Native Functions）**是游戏引擎自带的功能接口，比如显示消息、移动单位、造成伤害等。JASS本身只提供基础的语法规则，所有实际功能都要靠调用原生函数来实现。[^2]

#### 操作步骤

1. **查找你需要的原生函数** — 在触发器编辑器中点击「新建」→「新建自定义脚本」，然后输入函数名的前几个字母，按 **Ctrl+Space** 打开自动补全菜单，里面列出的就是原生函数[^5]
2. **了解函数的参数要求** — 比如 `GetUnitsInRangeOfLocAll(radius, location)` 需要两个参数：一个数字（半径范围）和一个点（位置）
...
> **💡 新手提示**：原生函数名通常以大驼峰命名法（每个单词首字母大写），比如 `GetTriggerUnit`、`SetUnitPosition`。记住这个规律可以帮助你快速识别原生函数。[^5]

> **⚠️ 常见错误**：新手经常混淆变量名和原生函数名。原生函数是游戏的内置功能，而变量是你自己创建的存储空间。调用原生函数用 `call`，读取变量则直接写变量名。

### 常见功能实现示例

下面我们来实现三个最常见的功能：**单位伤害、计时器、循环遍历**。这些是从GUI触发器迁移到JASS时最常用的场景。[^1]
...
> **💡 新手提示**：`GetTriggerUnit()` 是一个常用的原生函数，它返回触发这个事件的单位。很多GUI触发器中的"触发单位"在JASS里都要用这个函数来获取。[^1]
...
> **⚠️ 常见错误**：在循环中忘记写 `exitwhen u == null` 会导致无限循环，游戏会卡死。另外，创建的 `group` 变量记得最后用 `DestroyGroup` 销毁，否则会造成内存泄漏（内存占用越来越大）。[^1]

### 小结

完成本节学习后，你应该：
- ✅ 能够自己定义和调用一个简单的JASS函数
- ✅ 知道如何使用 `Ctrl+Space` 查看可用的原生函数
- ✅ 能够将基础的GUI触发逻辑翻译成JASS代码

**下一步建议**：尝试选择一个你之前用GUI触发器实现的功能，把它手动翻译成JASS脚本。这是最有效的学习方式！
...
1. **语法错误** — 这是最常见的错误类型[^1]
   - 缺少分号：`call Print("Hello")` 缺少分号会导致报错
   - 括号不匹配：每个左括号 `(` 必须有对应的右括号 `)`
   - 大小写错误：JASS区分大小写，`Function` 和 `function` 是不同的
   - 缺少引号或引号不配对
...
1. **使用打印调试法**[^1] — 这是最简单有效的调试方式
   - 在关键位置插入 `call BJDebugMsg("变量X的值是: " + I2S(变量X))`
   - 运行地图时，屏幕左下角会显示你设置的调试信息
   - 通过观察这些值的变化，判断代码执行到哪一步、变量值是否符合预期

2. **逐步注释法**
   - 先注释掉部分代码，只保留最简单的部分
   - 测试通过后，逐步取消注释，一步步增加复杂度
   - 这样能精确定位是哪一行代码出了问题

3. **加载测试**[^3]
   - 保存地图后，在World Editor中打开触发器编辑器
   - 如果有语法错误，地图加载时会弹出错误提示
   - 注意查看错误信息，它通常会告诉你出错的行号
...
**第三步：安装JASS专用编辑器提升效率**[^3]

推荐JASSCraft或JASS NewGen编辑器
...
[^3]: [[JASS Tutorials] - wc3edit.net](https://forum.wc3edit.net/viewtopic.php?t=11005) — accessed 2026-05-30
[^4]: [JASS: Basic - world-editor-tutorials.thehelper.net](https://world-editor-tutorials.thehelper.net/cat_usersubmit.php?view=25715) — accessed 2026-05-30
[^5]: [JASS Manual - SourceForge](https://jass.sourceforge.net/doc/) — accessed 2026-05-30
[^6]: [JASS: Introduction - World Editor Tutorials](https://world-editor-tutorials.thehelper.net/cat_usersubmit.php?view=21329) — accessed 2026-05-30